...
app.listen(8000);
setInterval(() => {
  http.get(`http://i-nvitemanager-alpha.glitch.me/`);//Glitch linkinizi doğru şekilde girin!
}, 280000)
///////////////
...
client.on("guildMemberRemove", async member => {
  ...
  if (!d) {
    const aa = new Discord.RichEmbed()
      .setColor("random")
      .setDescription(
        `\`\`${member.user.tag}\`\` **Adlı kullanıcı ayrıldı \nKullanıcıyı davet eden kişi:** \`\`Bulunamadı!\`\``
      )
      .setFooter(client.user.username, client.user.avatarURL);
    ...
    const aa = new Discord.RichEmbed()
      .setColor("BLACK")
      .setDescription(
        `\`\`${member.user.tag}\`\` **Adlı kullanıcı ayrıldı \nKullanıcıyı davet eden kişi:** \`\`${sa.tag}\`\``
      )
      .setFooter(client.user.username, client.user.avatarURL);
    client.channels.get(kanal).send(aa);
...
client.on("guildMemberAdd", async member => {
  member.guild.fetchInvites().then(async guildInvites => {
    ...
    const aa = new Discord.RichEmbed()
      .setColor("random")
      .setDescription(
        `\`\`${member.user.tag}\`\` **adlı Kullanıcı servera katıldı.\nKullanıcıyı davet eden** \`\`${davetçi.tag}\`\`\n**toplamda \`\`${sayı2}\`\` daveti olarak güncellendi**`
      )
      .setFooter(client.user.username, client.user.avatarURL);
    client.channels.get(kanal).send(aa);
    if (!veri) return;
